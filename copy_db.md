...
1. There will be a popup to finalize the process. Select the `Keep the prior system running without synchronization with the new system` option. This will keep your Standard/Enterprise instance running and stop ongoing synchronization with the Performance instance, creating a **copy** in the Performance plan. Both instances will be available for connections.
![Confirm upgrade button](images/confirm_complete_upgrade.png){: caption="Select the copy option and complete the process." caption-side="bottom"}
