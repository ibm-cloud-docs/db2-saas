...
{: #gh_restore-external-backup-performance}

This section explains how to restore your Perfomance plan database from an external backup in the new Genius Hub–enabled console. Follow these instructions if you see the updated UI. If you are still using the **legacy console**, refer to the [Restore from external backup](https://cloud.ibm.com/docs/db2-saas?topic=db2-saas-restore-external-backup-performance&interface=ui) section for the correct steps.
{: important}
...
## Prerequisites
{: #gh_restore-external-prerequisites}
{: ui}
...
## Prerequisites
{: #gh_restore-external-prerequisites-api}
{: api}
...
{: #gh_restore-external-create-backup}
{: ui}

1. Navigate to the External restore tab (**Administation > Databases > Select your database > Backup and recovery > Restore from external backup). If no internal backup exists, you will see a message indicating that an internal backup file is required.

   ![External restore page showing internal backup requirement](images/caas-ebr1.png){: caption="External restore page showing internal backup requirement" caption-side="bottom"}
...
3. Click the **Run backup** button and confirm by clicking **Run** in the dialog.
   ...
   ![Run backup confirmation dialog](images/run_backup.png){: caption="Run backup confirmation dialog" caption-side="bottom"}

4. Wait for the backup to complete. You can monitor the progress on the Backups page.

   ![Snapshot backup in progress](images/caas_sbackup.png){: caption="Snapshot backup in progress" caption-side="bottom"}

## Create internal backup (Prerequisite)
{: #gh_restore-external-create-backup-api}
{: api}
...
2. Click the **External restore** button to begin the restore wizard.
   ...
   ![External restore page ready to initiate restore](images/acess_ir.png){: caption="External restore page ready to initiate restore" caption-side="bottom"}

### Step 1: Connect external system
{: #gh_restore-external-connect}
...
6. Click **Next** to proceed.
   ...
   ![Connect external system - entering storage credentials](images/connect_exsys.png){: caption="Connect external system by entering storage credentials" caption-side="bottom"}

### Step 2: Select backup image
{: #gh_restore-external-select-backup}
...
4. Click **Next** to proceed to the confirmation step.
   ...
   ![Select backup image - browsing bucket folders](images/restore_exbckup.png){: caption="Select backup image" caption-side="bottom"}

### Step 3: Confirm
{: #gh_restore-external-confirm}
...
1. Choose whether to perform backup size validation or skip it. Backup size validation checks if your instance has sufficient storage space for the backup before starting the restore. Skipping validation will start the restore immediately, but it may fail mid-process if storage is insufficient.
   ...
   ![Confirm restore backup with validation and rollforward options](images/confirm.png){: caption="Confirm restore backup with validation and rollforward options" caption-side="bottom"}
...
Once the restore is initiated, Users can see a patch screen indicating that the external restore is in progress.

![External restore in precheck status](images/rest_ext_monitor-1.png)

![External restore in precheck status](images/rest_ext_mon-2.png)
...
1. When the restore is complete, the status will show "Completed" with a green checkmark.
   ...
   ![External restore completed successfully](images/caas_ebr_completed.png){: caption="External restore completed successfully" caption-side="bottom"}
...
2. You can expand the restore entry to view the rollforward history, which shows details of each rollforward operation performed.
   ...
   ![Completed restore with rollforward history expanded](images/verify_comp2.png){: caption="Completed restore with rollforward history expanded" caption-side="bottom"}
